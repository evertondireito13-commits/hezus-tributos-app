...
export default function LeadForm() {
  ...
  return (
    <section id="lead" className="relative overflow-hidden border-t border-line bg-white/[0.02] py-24">
      <img src="/hezus-mark.png" alt="" aria-hidden="true" className="pointer-events-none absolute -left-20 -bottom-16 hidden h-96 w-auto rotate-12 opacity-[0.05] md:block" />
      <div ref={ref} className="reveal reveal-init relative mx-auto max-w-2xl px-6 text-center">
        ...
        {sent ? (
          ...
          <form onSubmit={handleSubmit} className="mt-8 grid gap-4 text-left sm:grid-cols-2">
            ...
            <button type="submit" className="sm:col-span-2 mt-2 rounded-full bg-blue px-7 py-3 font-semibold text-white transition hover:brightness-110">Quero fazer meu diagnóstico</button>
            <a href="https://wa.me/5500000000000?text=Ol%C3%A1!%20Quero%20entrar%20em%20contato%20com%20a%20Hezus%20Capital%20e%20Tributos." target="_blank" rel="noreferrer" className="sm:col-span-2 rounded-full border border-line px-7 py-3 text-center text-sm font-medium text-ice/70 transition hover:border-ice/40 hover:text-ice">Prefere WhatsApp? Entrar em contato agora</a>
          </form>
        )}
      </div>
    </section>
