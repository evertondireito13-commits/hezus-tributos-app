  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-line bg-graphite/85 backdrop-blur">
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3 sm:px-6 sm:py-4">
        ...
        <div className="hidden items-center whitespace-nowrap md:flex md:space-x-4 lg:space-x-7 xl:space-x-8">
          {LINKS.map((l) => (
            <a key={l.href} href={l.href} className="text-sm text-ice/70 transition hover:text-ice">{l.label}</a>
          ))}
        </div>
        <a href="#lead" className="hidden shrink-0 rounded-full bg-blue px-5 py-2 text-sm font-semibold text-white transition hover:brightness-110 md:ml-6 md:inline-block">Diagnóstico gratuito</a>
